...
export function apiHandler(service, { auth = false, registration = false, roles, schema, authLimit = false } = {}) {
    ...
    try {
      ...
      if (registration) {
        if (await User.countDocuments() !== 0) {
          ...
          try { await protect(req, res); } catch { /* validation/controller still execute */ }
        }
