...
function fixture(name, extras = {}) {
  const body = { firstName: 'SyntheticAuth', lastName: name, email: `${prefix}-${name}@example.invalid`, password, ...extras };
...
const alter = (user, changes) => User.collection.updateOne({ _id: new mongoose.Types.ObjectId(user._id), email: user.email }, { $set: changes });

test('backend authentication contract against the isolated local database', { timeout: 240000 }, async t => {
  ...
  t.after(async () => {
    try {
      if (emptyConfirmed) {
        ...
        assert.equal(await User.countDocuments(), 0, 'All task synthetic records must be removed');
        t.diagnostic(`Cleaned only ${records.size} tracked synthetic identities with prefix ${prefix}; users count restored to zero.`);
      }
    } finally {
  ...
  emptyConfirmed = true;
  const indexesBefore = await User.collection.indexes().catch(error => { if (error.code === 26) return []; throw error; });
  server = spawn(process.execPath, ['node_modules/next/dist/bin/next', 'dev', '--hostname', '127.0.0.1', '--port', '3104'], {
    cwd: process.cwd(), windowsHide: true, env: { ...process.env, NODE_ENV: 'development' }, stdio: ['ignore', 'pipe', 'pipe'],
  });
  ...
  for (let i = 0; i < 90; i++) {
    ...
    try { ready = (await fetch(`${base}/api/health`)).ok; } catch { /* wait for the owned server */ }
    if (ready) break;
    await delay(500);
  }
  assert.ok(ready, 'Test server must start');

  await t.test('A14/A19 first-user validation, forced admin, exact response and model defaults', async () => {
    status(await post('/auth/register', { ...fixture('weak'), password: 'weak', role: 'staff' }), 400, 'Validation failed');
    status(await post('/auth/register', fixture('bad-role', { role: 'unknown' })), 400, 'Validation failed');
    const body = fixture('admin', { role: 'staff', permissions: ['manage_users'], isActive: false, nic: 'discard', address: 'discard' });
    const result = await post('/auth/register', body);
    admin = status(result, 201, 'Admin account created successfully. Please login.');
    assert.equal(admin.role, 'admin');
    assert.deepEqual(Object.keys(admin).sort(), ['_id', 'email', 'firstName', 'fullName', 'lastName', 'role'].sort());
  ...
  await t.test('A01/A26 valid login, id-only JWT claims and seven-day expiry', async () => {
    ...
    assert.deepEqual(Object.keys(data).sort(), ['_id', 'firstName', 'lastName', 'fullName', 'email', 'role', 'lastLogin', 'token'].sort());
  ...
  await t.test('A15/A17/A18 registration gates and swallowed-auth error precedence', async () => {
    status(await post('/auth/register', fixture('unauthorized')), 401, 'Not authorized. Only existing admins can register new users.');
    status(await post('/auth/register', { email: 'bad' }), 400, 'Validation failed');
    status(await post('/auth/register', { ...fixture('duplicate'), email: admin.email }), 400, 'User with this email already exists');
    staff = status(await post('/auth/register', fixture('staff'), adminToken), 201, 'User registered successfully');
    ...
    status(await post('/auth/register', fixture('forbidden'), staffToken), 403, 'Only admins can register new users');
    status(await post('/auth/register', { ...fixture('duplicate-staff'), email: admin.email }, staffToken), 400, 'User with this email already exists');
    customer = status(await post('/auth/register', fixture('customer', { role: 'customer' }), adminToken), 201);
    customerToken = status(await login(customer.email), 200).token;
  });
  await t.test('A12/A13/A20 Zod mismatch, normalization, stripped fields and model validation', async () => {
    status(await post('/auth/login', { email: 'bad', password }), 400, 'Validation failed');
    status(await post('/auth/login', { email: ` ${admin.email} `, password }), 400, 'Validation failed');
    status(await post('/auth/login', { email: admin.email.toUpperCase(), password, role: 'customer' }), 200);
    status(await post('/auth/register', fixture('blank', { firstName: '   ' }), adminToken), 400, 'First name is required');
  });
  await t.test('A02 unknown email login', async () => {
    status(await login(`${prefix}-absent@example.invalid`), 401, 'Invalid email or password');
  });
  await t.test('A03/A04/A05/A09/A84 five failures lock for fifteen minutes; tokens and hash survive', async () => {
    const before = await read(staff);
    for (let attempt = 1; attempt <= 5; attempt++) {
      const start = Date.now();
      const result = await post('/auth/login', { email: staff.email, password: 'incorrect' });
      status(result, attempt === 5 ? 423 : 401, attempt === 5 ? 'Account locked due to too many failed attempts. Try again in 15 minutes.' : `Invalid email or password. ${5 - attempt} attempt(s) remaining.`);
      const user = await read(staff);
      assert.equal(user.failedLoginAttempts, attempt);
      assert.equal(user.password, before.password);
      if (attempt === 5) assert.ok(user.lockedUntil >= start + 900000 && user.lockedUntil <= Date.now() + 900000);
    }
  ...
  await t.test('A06/A07/A08 inactive ordering, expired-lock relock and success reset', async () => {
    ...
    status(await request('/auth/me', { token: staffToken }), 401, 'Not authorized, token invalid or expired');
    assert.equal((await read(staff)).failedLoginAttempts, 5);
  ...
  await t.test('A27/A28/A29/A30 bearer quirks, expired/bad/absent identities', async () => {
    for (const header of ['', `bearer ${adminToken}`, 'Bearer', `Bearer  ${adminToken}`]) {
      status(await request('/auth/me', { headers: { Authorization: header } }), 401, 'Not authorized, no token provided');
    }
    status(await request('/auth/me', { headers: { Authorization: `BearerExtra ${adminToken}` } }), 200);
    const invalid = ['bad', jwt.sign({ id: admin._id }, randomBytes(32), { expiresIn: '1h' }), jwt.sign({ id: admin._id }, process.env.JWT_SECRET, { expiresIn: -1 }), generateToken(new mongoose.Types.ObjectId()), generateToken('bad-id')];
    for (const token of invalid) status(await request('/auth/me', { token }), 401, 'Not authorized, token invalid or expired');
  });
  await t.test('A40 current user and unrestricted user detail include virtuals, never password', async () => {
  ...
  await t.test('A35/A36/A37/A38 password-change rules, hashing and no counter/token reset', async () => {
    const before = await read(staff);
    for (const value of ['', 'abcde', 'abcdef', 'abcdefg']) {
      const message = !value ? 'Both current and new passwords are required' : value.length < 6 ? 'New password must be at least 6 characters' : 'Password must be at least 8 characters';
      status(await post('/auth/change-password', { currentPassword: password, newPassword: value }, staffToken), 400, message);
    }
    status(await post('/auth/change-password', { currentPassword: 'incorrect', newPassword: 'lowercaseonly' }, staffToken), 401, 'Current password is incorrect');
    assert.equal((await read(staff)).password, before.password);
    await alter(staff, { failedLoginAttempts: 2 });
    status(await post('/auth/change-password', { currentPassword: password, newPassword: 'lowercaseonly' }, staffToken), 200, 'Password changed successfully');
  ...
  await t.test('A60/A61/A62/A64/A65 admin verification ignores alternate email and admin lock', async () => {
    status(await post('/auth/verify-admin', {}, staffToken), 400, 'Admin password is required');
    status(await post('/auth/verify-admin', { password }, staffToken), 403, 'Not authorized as admin. Please provide admin credentials.');
    status(await post('/auth/verify-admin', { password: 'incorrect' }, adminToken), 401, 'Invalid password');
    const own = status(await post('/auth/verify-admin', { password, adminEmail: 'ignored@example.invalid' }, adminToken), 200, 'Admin verification successful');
    ...
    const before = await read(admin);
    const other = status(await post('/auth/verify-admin', { password, adminEmail: admin.email }, staffToken), 200);
    ...
    status(await post('/auth/verify-admin', { password: 'wrong', adminEmail: admin.email }, staffToken), 401, 'Invalid admin credentials');
    await alter(admin, { isActive: false });
    status(await post('/auth/verify-admin', { password, adminEmail: admin.email }, staffToken), 401, 'Invalid admin credentials');
    await alter(admin, { isActive: true });
  });
  await t.test('A41/A42 soft-delete visibility, count discrepancy and regex filters', async () => {
    const hidden = await direct('hidden', { deletedAt: new Date(), role: 'customer' });
    const hiddenToken = generateToken(hidden._id);
    status(await request('/auth/me', { token: hiddenToken }), 401, 'Not authorized, token invalid or expired');
    ...
    const list = await request('/users?role=customer&search=^Synthetic&limit=1&page=1', { token: customerToken });
    status(list, 200);
    assert.deepEqual(Object.keys(list.body).sort(), ['success', 'count', 'total', 'data'].sort());
    assert.equal(list.body.count, 1);
    assert.equal(list.body.total, 2);
    const inactive = await direct('inactive', { isActive: false });
    const result = await request('/users?isActive=anything&search=inactive', { token: staffToken });
    status(result, 200);
    assert.equal(result.body.data[0]._id, String(inactive._id));
    assert.equal(result.body.data[0].password, undefined);
  });
  await t.test('A32/A43/A70 all ten roles: reads allowed, user writes admin-only; permissions ignored', async () => {
    for (const role of User.schema.path('role').enumValues) {
      ...
      const result = await request(`/users/${customer._id}`, { method: 'PUT', token, body: { phone: '123' } });
      status(result, role === 'admin' ? 200 : 403, role === 'admin' ? undefined : `Role '${role}' is not authorized for this action`);
      if (role !== 'admin') status(await request(`/users/${customer._id}`, { method: 'DELETE', token }), 403);
    }
  });
  await t.test('A44 admin update uses exactly five fields and preserves password hash', async () => {
    const before = await read(customer);
    const result = await request(`/users/${customer._id}`, { method: 'PUT', token: adminToken, body: { lastName: 'updated', phone: ' 555 ', role: 'manager', isActive: true, email: 'discard@example.invalid', password: 'Discarded9', permissions: ['manage_users'], nic: 'discard', address: 'discard' } });
    ...
    status(await request(`/users/${customer._id}`, { method: 'PUT', token: adminToken, body: { role: 'unknown' } }), 400);
  });
  await t.test('A31/A45 own-role and own-deactivation changes immediately affect existing JWT', async () => {
    const actor = await direct('mutable-admin', { role: 'admin' });
    const token = generateToken(actor._id);
    status(await request(`/users/${actor._id}`, { method: 'PUT', token, body: { role: 'staff' } }), 200);
    status(await request(`/users/${actor._id}`, { method: 'PUT', token, body: { role: 'admin' } }), 403);
    await alter(actor, { role: 'admin' });
    status(await request(`/users/${actor._id}`, { method: 'PUT', token, body: { isActive: false } }), 200);
    status(await request('/users', { token }), 401, 'Not authorized, token invalid or expired');
  });
  await t.test('A46/A47 self-delete guard, permanent deletion, invalid and absent IDs', async () => {
    status(await request(`/users/${admin._id}`, { method: 'DELETE', token: adminToken }), 400, 'Cannot delete yourself');
    const actor = await direct('delete-me');
    const token = generateToken(actor._id);
    status(await request(`/users/${actor._id}`, { method: 'DELETE', token: adminToken }), 200, 'User deleted permanently from database');
    assert.equal(await User.collection.findOne({ _id: actor._id, email: actor.email }), null);
    status(await request('/users', { token }), 401, 'Not authorized, token invalid or expired');
    for (const method of ['GET', 'PUT', 'DELETE']) {
      status(await request('/users/bad-id', { method, token: adminToken, ...(method === 'PUT' ? { body: {} } : {}) }), 404, 'Resource not found');
      status(await request(`/users/${new mongoose.Types.ObjectId()}`, { method, token: adminToken, ...(method === 'PUT' ? { body: {} } : {}) }), 404, 'User not found');
    }
  });
  await t.test('A26/A80 JWT fallback and response helper characterization without exposing secrets', async () => {
    ...
    } finally { if (lifetime === undefined) delete process.env.JWT_EXPIRES_IN; else process.env.JWT_EXPIRES_IN = lifetime; }
    const env = process.env.NODE_ENV;
    try {
      process.env.NODE_ENV = 'production';
      const err = Object.assign(new Error('duplicate'), { code: 11000, keyValue: { email: 'synthetic@example.invalid' } });
      const response = errorResponse(err, createResponse());
      assert.equal(response.status, 400);
      assert.deepEqual(await response.json(), { success: false, message: 'Duplicate email: synthetic@example.invalid already exists', stack: null });
      process.env.NODE_ENV = 'development';
      assert.equal(typeof (await errorResponse(new Error('synthetic failure'), createResponse()).json()).stack, 'string');
  ...
  await t.test('A79 actual peer limiter is shared across auth routes, ignores spoofed IP and excludes users', async () => {
    let result = await request('/auth/me', { token: adminToken });
    status(result, 200);
    const remaining = Number(result.headers.get('ratelimit-remaining'));
    assert.ok(remaining > 0);
    result = await request('/auth/me', { token: adminToken, headers: { 'X-Forwarded-For': '203.0.113.8' } });
    status(result, 200);
    ...
    for (const path of ['/auth/me', '/auth/logout', '/auth/register', '/auth/login', '/auth/change-password', '/auth/verify-admin']) {
      const blocked = await request(path, { method: path === '/auth/me' ? 'GET' : 'POST', token: adminToken, ...(path === '/auth/me' ? {} : { body: {} }) });
  ...
  await t.test('no email index created or modified during authentication tests', async () => {
    ...
    assert.deepEqual(after.filter(index => index.name !== '_id_'), indexesBefore.filter(index => index.name !== '_id_'));
  });
});
